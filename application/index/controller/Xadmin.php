...
class Xadmin extends Controller{
	...
	public function videoRecord($name) {
		...
		foreach ($result as $data) {
			...
			$videoH5 .= '<ul class=vdcv id=vdcv><li class=vdcvli><a href=' . $videoResult["src"] . ' title=' . $videoResult["title"] . ' target=_blank class=vdcvimg><img class=vdcvloading data-original=' .  $videoResult["cover"] . 'alt=' . $videoResult["title"] . 'style=display:inline src=' . $videoResult["cover"] . '><span class=vdcvmask style=opacity:0;><i class=glyphicon glyphicon-play-circle glyphiconL></i></span></a><div class=vdcvinfo><a  href=' . $videoResult["src"] . ' target=_blank>' . $videoResult["title"] . '</a><p><span class=vdcvf1>更新至' . $cotdir . '集</span></p></div></li></ul>';
		}
