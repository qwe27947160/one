...
class Test 
{
    public function run(&$compress)//压缩HTML
    {
        /*
        $find = '/>\s+</';
        $replace = '><';
        $params = preg_replace($find, $replace, $params);
		*/
    $chunks   = preg_split('/(<!--<nocompress>-->.*?<!--<\/nocompress>-->|<nocompress>.*?<\/nocompress>|<pre.*?\/pre>|<textarea.*?\/textarea>|<script.*?\/script>)/msi', $compress, -1, PREG_SPLIT_DELIM_CAPTURE);
    $compress = '';
    foreach ($chunks as $c) {
